...
function ImageGrid() {
  ...
  const handleQAGenerator = (img_id) => {
    console.log(img_id)
    axios.get('/qa_generator/' + img_id).then(res => {
      console.log(img_id)
    })
        ...
        <div className='image-grid'>
          {images.map(image =>(
            <div class="card m-2" style={{width: "450px", height:"900px"}}>
              ...
              <div class="card-body">
                <h5 class="card-title text-center">Question-Answer</h5>
                {image.qa == "" ? (
                  <div class="text-center">
                  <button class="btn btn-primary btn-sm m-2 " classkey={image.img_id} onClick={() => handleQAGenerator(image.img_id)}>
                    QA Generate
                  </button>
                  </div>
                ):(<span style={{"white-space":"pre-line"}}>{image.qa}</span>)}
