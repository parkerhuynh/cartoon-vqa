...
function ImageGrid() {
  ...
  const handleSubmit = (event) => {
    ...
    if (searchText =="") { 
      axios.get('/get_triples/random').then(res => {
        setImages(res.data);
      })
    } else {
      axios.get('/get_triples/' + searchText).then(res => {
        setImages(res.data);
      })
    }
  ...
  const handlRejecteChange = (worker_id, assignment_id) => {
    axios.get('/delete_assigment_worker_tracking/' + worker_id + '/' + assignment_id).then(res => {
      setImages(res.data);})
  };

  const handlApprovelChange = (worker_id, assignment_id) => {
    axios.get('/approve_assigment_worker_tracking/' + worker_id + '/' + assignment_id).then(res => {
      setImages(res.data);})
  };
